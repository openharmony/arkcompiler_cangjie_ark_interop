...
def build():
    ...
    entry_default = tb.save(test_project.get_signed_hap_file_path(), 'entry-default.hap')
    tb.install(entry_default)

    test_project.build(build_mode='test')
    test_project.do_signing(name='ohosTest')
    entry_ohosTest = tb.save(test_project.get_ohos_test_signed_hap_file_path(), 'entry-ohosTest.hap')
    tb.install(entry_ohosTest)

    tb.dump_json(project_type='hybrid')
