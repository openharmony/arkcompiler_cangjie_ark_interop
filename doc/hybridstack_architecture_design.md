...
2. **互操作侧已存在 BusinessException 包装层**：`ohos/business_exception/business_exception.cj` 与 `ohos/ark_interop/js_exception.cj` 中 `toJSError()` 在边界处构造新的 ArkTS Error，覆盖 PC Array，是当前 faultlog 丢失仓颉帧的根因。
...
flowchart LR
    ...
    subgraph Napi[arkui_napi]
        CJFFI["cjffi/ark_interop_napi.h<br/>ARKTS_UpdateStackInfo"]
    ...
    IFCallbacks -->|opKind=PUSH<br/>当前帧 PC| CJFFI
    CJFFI -->|更新 PcVector| VM

    JsExc -->|opKind=PUSH<br/>跨边界帧| CJFFI
    JsExc -->|throw 到 ArkTS| ETSApp
...
sequenceDiagram
    ...
    alt 复用 pendingJSError 且 fromInteropBoundary==true
        Note over JS: 不创建新 Error → PcVector 保持不变
    else 必须创建新 wrapper（首次跨边界）
        JS->>CFFI: ARKTS_GetPcVectorSnapshot(vm, &snap)
        JS->>BIZ: 构造 BusinessException(CrossCangjie)
        Note over BIZ: 创建 ArkTS Error → PcVector 被覆盖
        JS->>CFFI: ARKTS_RestorePcVectorSnapshot(vm, snap)
        Note over JS: PcVector 恢复到仓颉报错点
    end
    JS->>ETS: ARKTS_Throw(error)
...
├── ohos/
│   ├── ark_interop/
│   │   ├── js_exception.cj            # 重构：toJSError 不再无条件覆盖 PC
│   │   └── js_module.cj               # 重构：注册新 stack-info opKind
│   ├── business_exception/
...
// 仅在仓颉运行时构造异常时调用（场景 A）
// opKind 复用 cjffi 已有定义（PUSH=0 / POP=1）
extern "C" void CJ_HybridStack_PushCJFrames(unsigned long long vmAddr,
                                            void* subStackInfo,
                                            unsigned int opKind);
```
...
- `CJ_HybridStack_PushCJFrames` → `ARKTS_UpdateStackInfo`（faultlog 路径）
...
| 文件 | 现状 | 改动 |
| ---- | ---- | ---- |
| `ohos/ark_interop/js_exception.cj:145-155 createJSError` | 每次跨边界都创建 ArkTS Error，覆盖 PcVector | 增加分支：若 `pendingJSError` 已记录原始仓颉异常对应的 wrapper 且 `SharedException.fromInteropBoundary == true`，则复用；新建 wrapper 时按顺序调用 `ARKTS_GetPcVectorSnapshot` → 构造 `BusinessException` → `ARKTS_RestorePcVectorSnapshot`，保护 PcVector 不被覆盖（参见 §4.2.4） |
| `ohos/ark_interop/js_module.cj:36-43 CJModuleCallbacks` | 仅有 `throwJSError` 回调 | 新增 `onCJExceptionCreated` 回调，由 cangjie_runtime 在异常对象构造时回调互操作（**需要 cangjie_runtime 配合提供调用点**——若运行时短期无法配合，回退到「在跨边界 `toJSError` 时一次性 push 仓颉帧」的等效方案） |
...
| 场景 | 处理 |
| ---- | ---- |
| 仓颉抛出异常对象（场景 A） | 取仓颉当前帧 backtrace → `ARKTS_PushCJFramesToPcVector` 直注 |
| 互操作 wrapper Error 创建前（场景 B） | `ARKTS_GetPcVectorSnapshot` 保存当前 PcVector |
| Wrapper Error 创建后 | `ARKTS_RestorePcVectorSnapshot` 立即恢复，避免 wrapper 自身的 PC 覆盖原始仓颉 PC |
| 复用 `pendingJSError`（场景 B 优化路径） | 不创建新 Error，无需快照-恢复 |
...
2. **回调时序**：cangjie_runtime 必须在异常**对象**构造时（而非 throw 时）回调，否则栈帧已开始 unwind，PC 无法准确捕获。需与仓颉运行时 owner 对齐。若运行时无构造回调，使用降级路径：在跨边界 `toJSError` 入口处主动调用仓颉 backtrace API（候选：`std.runtime.Backtrace.current()` / `Thread.currentThread().getStackTrace()`，由 plan Task 0 spike 锁定具体 API 名）一次性补齐。
...
| ArkTS Wrapper 与 PC 覆盖 | 未处理，依赖 stitching 弥补 | 显式 BARRIER / 复用 `pendingJSError` 杜绝覆盖 |
...
1. 与 ets_runtime / arkui_napi owner 对齐 `opKind=BARRIER` 接收度（plan Task 1 含 spike）。
2. 与 cangjie_runtime owner 对齐异常构造回调点（plan Task 2 含 spike）。
