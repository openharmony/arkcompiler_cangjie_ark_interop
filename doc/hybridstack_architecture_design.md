...
| **C. cangjie_runtime 新增 `onCJExceptionCreated` 回调** | **异常对象构造瞬间** | ✓ 仓颉栈仍在；写 PcVector 即可被 faultlog 读取 | ✓ catch 路径同样触发 | **采用** |
...
由于 `createJSError` 必然覆盖 PcVector，单点写入无法兼顾「faultlog」与「toJSError 后的仓颉侧读取」。本设计采用**二阶段写入**：

- **阶段 ①（异常构造期，运行时回调中）**：互操作回调获取仓颉 PC → 备份至 `SharedException.cjPcSnapshot` → 经 cjffi 中转写入当前 vm PcVector。faultlog 路径在此后任何时刻读取均可见仓颉帧。
- **阶段 ②（互操作 `toJSError` 内）**：`createJSError` 创建新 JSError 并覆盖 PcVector → **立即**用 `cjPcSnapshot` 再次写入 PcVector 恢复。后续仓颉 API 读到的就是恢复后的 PcVector。
...
#### 4.1.3 arkui_napi cjffi（新增中转函数）

互操作不直接链接 `libark_jsruntime`，对 ets_runtime 的访问统一经 `interfaces/inner_api/cjffi/ark_interop` 中转。本设计在该模块新增**一个中转函数**：

```c
// arkui_napi/interfaces/inner_api/cjffi/ark_interop/ark_interop_napi.h（新增）
// 内部解出 napi_env 对应的 EcmaVM，再调 DFXJSNApi::UpdateHybridStackTracePc
int ARKTS_UpdateHybridStackTracePc(napi_env env, void** frames, int count);
```

互操作侧的 `CJ_HybridStack_UpdatePc` 即为对 `ARKTS_UpdateHybridStackTracePc` 的薄封装。
...
| `ohos/ark_interop/js_exception.cj` | `SharedException` 增加字段 `cjPcSnapshot: ?Array<UIntNative>`；`toJSError` 在 `createJSError` 之后立即调用 `CJ_HybridStack_UpdatePc` 用快照恢复 PcVector |
...
| `ohos/hybrid_stack/hybrid_stack.cj`（新增） | 仓颉侧 API：`HybridStack.getTrace(env)` |
| `frameworks/native/hybrid_stack/hybrid_stack_bridge.{h,cpp}`（新增） | C 桥接：`CJ_HybridStack_UpdatePc` → `ARKTS_UpdateHybridStackTracePc`；`CJ_HybridStack_GetPcVector` → 经 cjffi 取当前 vm 的 PcVector |

**onCJExceptionCreated 回调内部行为**：
1. 通过公开 HiDebug API 采集仓颉 PC 帧（`OH_HiDebug_CreateBacktraceObject` + `OH_HiDebug_BacktraceFromFp`）；
2. 备份 PC 数组到对应 `SharedException.cjPcSnapshot`；
3. 经 cjffi 中转调用 `ARKTS_UpdateHybridStackTracePc(env, frames, n)` 写入当前 vm PcVector。

**仓颉侧符号化路径**：`HybridStack.getTrace` 直接使用 HiDebug 公开 API 自行解析，**不依赖 `napi_get_hybrid_stack_trace`**。流程：经 cjffi 取当前 vm 的 PcVector → 对每个 PC 调用 `OH_HiDebug_SymbolicAddress` 得到 `(file, function, line)` → 按帧来源（Cangjie / ArkTS / Native）分段拼装字符串。

### 4.2 不改动、仅依赖其既有行为的组件

| 组件 | 在本方案中的角色 | 是否改动 |
| ---- | ---- | ---- |
| arkui_napi cjffi（除 §4.1.3 新增中转外） | 既有 ark_interop 系列封装继续承担 `napi_env ↔ EcmaVM` 桥接 | 否 |
| ets_runtime（除 DFXJSNApi 新增外） | `EcmaVM` 持有 PcVector；faultlog 路径既有的混合栈解析继续生效 | 否 |
| HiDebug | 提供公开栈回溯（PC 采集）与符号化 API，**互操作直接调用** | 否 |
...
```mermaid
flowchart LR
    subgraph User["应用代码（多层调用）"]
        ETSTop["ArkTS 顶层 / 应用入口"]
        ETSMid["ArkTS 中间层"]
        CJTop["Cangjie 顶层（被 ArkTS 调用）"]
        CJBot["Cangjie 内层（throw 点）"]
        ETSTop --> ETSMid --> CJTop --> CJBot
    end

    subgraph CJRT["cangjie_runtime（本设计要求新增 onCJExceptionCreated 触发点）"]
        CJExc["异常对象构造收尾"]
    end

    subgraph Interop["arkcompiler_cangjie_ark_interop（本仓库改动）"]
        IFCB["onCJExceptionCreated 实现"]
        ToJS["toJSError 跨边界转换<br/>（createJSError + 立即恢复 PcVector）"]
        Biz["BusinessException.toString /<br/>getMixedStackTrace（仓颉 API）"]
    end

    subgraph Napi["arkui_napi cjffi（新增中转函数）"]
        ArkRelay["ARKTS_UpdateHybridStackTracePc（新增）"]
        ArkRelay2["ARKTS_GetHybridStackTraceStr（新增）"]
        ArkRelay3["ARKTS_GetHybridStackTracePc（新增）"]
    end

    subgraph EtsRT["ets_runtime（DFXJSNApi 新增 1 个 API）"]
        VM["EcmaVM PcVector"]
        UpdPc["DFXJSNApi::UpdateHybridStackTracePc（新增）"]
    end

    subgraph External["HiDebug"]
        HiDbg["HiDebug 公开 API（PC 采集 + 符号化）"]
    end
    subgraph External2["faultloggerd"]
        Fault["faultloggerd / signalhandler / dfx_dump_catcher"]
    end
    
    CJBot -->|throw| CJExc
    CJExc -->|构造时回调| IFCB
    IFCB -->|2. 备份 cjPcSnapshot + 写 PcVector| ArkRelay
    ArkRelay --> UpdPc --> VM
    ToJS -->|抛 JSError 给 ArkTS| ETSMid
    CJTop -->|catch 后调用仓颉 API| Biz
    IFCB -->|1. 经 cjffi 取 PcVector| ArkRelay3
    ArkRelay3 -->| 通过公开API获取 PcVector| HiDbg
    Biz -->|3. 对每个 PC 符号化| ArkRelay2
    ArkRelay2 -->| 通过公开API对每个 PC 符号化| HiDbg
    Fault -->|进程崩溃读 vm PcVector| VM

    classDef new fill:#fff5cc,stroke:#d4a017,stroke-width:2px;
    class IFCB,ToJS,Biz,ArkRelay,ArkRelay2,ArkRelay3,UpdPc new
    classDef runtime_change fill:#ffe0e0,stroke:#c0392b,stroke-width:2px;
    class CJExc runtime_change
```

> 图例：黄色 = 互操作 / arkui_napi cjffi / ets_runtime 新增；红色 = 本设计要求 cangjie_runtime 新增触发点；其余为既有组件，仅依赖其行为。
...
sequenceDiagram
    ...
    IF->>IF: HiDebug 采 PC + 备份 cjPcSnapshot
    IF->>Cjffi: ARKTS_UpdateHybridStackTracePc(env, frames)
    Cjffi->>VM: 写入 PcVector
...
sequenceDiagram
    ...
    IF->>Cjffi: 写 PcVector + 备份 cjPcSnapshot
    Cjffi->>VM: 更新 PcVector
    CR->>TJ: 越界
    TJ->>VM: createJSError（覆盖 PcVector）
    TJ->>Cjffi: 立即用 cjPcSnapshot 恢复
    Cjffi->>VM: 恢复 PcVector
    Note over TJ,CJTop: 异常被仓颉调用方捕获（直接拿到 BusinessException 对象）
    CJTop->>Biz: e.toString() / e.getMixedStackTrace()（仓颉 API）
    Biz->>Cjffi: 经 cjffi 取当前 vm PcVector
    Cjffi-->>Biz: PC 数组
    Biz->>HiDbg: OH_HiDebug_SymbolicAddress（按帧符号化）
    HiDbg-->>Biz: (file, function, line)
    Biz-->>CJTop: 返回「Cangjie + ArkTS (+ Native)」分段字符串
```
...
```c
// arkui_napi/interfaces/inner_api/cjffi/ark_interop/ark_interop_napi.h（新增中转）
int ARKTS_UpdateHybridStackTracePc(napi_env env, void** frames, int count);
```

```cpp
// 本仓库 frameworks/native/hybrid_stack/hybrid_stack_bridge.h
extern "C" int CJ_HybridStack_UpdatePc(napi_env env, void** frames, int count);
extern "C" int CJ_HybridStack_GetPcVector(napi_env env, void** outBuf, int* outCount);
```

```cangjie
// 本仓库 ohos/hybrid_stack/hybrid_stack.cj（仓颉 API）
public class HybridStack {
    // 取当前 vm PcVector，对每个 PC 经 HiDebug 符号化，输出分段混合栈
...
3. **arkui_napi cjffi 中转函数落地**：`ARKTS_UpdateHybridStackTracePc` 必须被 napi 团队接受；否则需另寻互操作访问 `EcmaVM` 的途径。
...
| ---- | ---- | ---- |
| Faultlog 仓颉栈来源 | 互操作侧字符串拼接注入 | ets_runtime PcVector，由 cangjie_runtime 新增回调 + 互操作经 cjffi 写入 |
| 跨边界 PC 覆盖处理 | 无 | `toJSError` 内立即用 `cjPcSnapshot` 恢复 |
| 仓颉 API 混合栈 | 字符串拼接 | `HybridStack.getTrace` 直接基于 PcVector + HiDebug 符号化，**不依赖 `napi_get_hybrid_stack_trace`** |
| 多 VM 支持 | 未考虑 | 每 vm 独立，天然支持 |
| 对仓颉运行时依赖 | 强（自带 backtrace 抓取与拼接） | **明确要求新增一个构造期回调**，PC 采集与写入由互操作完成 |
...
3. 与 arkui_napi owner 对齐 `ARKTS_UpdateHybridStackTracePc` 中转函数。
