...
>
> hdc server运行时，默认会监听2in1的8710端口，开发者可通过设置系统环境变量OHOS_HDC_SERVER_PORT自定义监听的端口号。
...
hdc server启动时，默认会监听2in1的8710端口，hdc client使用tcp协议通过此端口连接server。如果2in1的8710端口已经被使用或者希望使用其他端口，可以通过添加环境变量OHOS_HDC_SERVER_PORT到系统环境变量中来修改server启动时监听的端口号。
...
在使用hdc前，请在设备上开启usb调试功能，用usb线连接设备和2in1。
...
| USB数据连接线 | 使用USB数据连接线连接到调试2in1的USB接口 | 如使用低带宽、无数据通信功能的USB连接线可能导致无法识别HDC设备，建议更换官方USB数据连接线。 |
| USB接口 | 主板直出USB接口（台式机为后面板的USB接口，笔记本为机身的USB接口） | 如使用转接头/拓展坞/台式机前面板USB接口，存在带宽低和USB同步异常等问题，会导致频繁断连，推荐使用直连方式连接2in1和设备。 |
...
| 确认项 | 正常 | 异常处理 |
| -------- | -------- | -------- |
| 网络连接 | 2in1、手机设备处于同一网络。 | 连接同一WiFi或手机开启热点。 |
...
远程连接场景是指客户端通过网络远程连接服务端，客户端和服务端在不同的2in1运行，服务端连接设备。
远程连接如图所示:

![远程连接结构图](./figures/hdc_image_004.PNG)

hdc client（客户端）在2in1 1中运行，hdc server（服务端）在2in1 2中运行，2in1 2中的hdc server连接设备。
...
1. 打开设备网络连接通道，命令格式如下：
   ...
   **使用方法：**
   ...
   > **注意：**
   >
   > 切换前，请确保条件满足：远端设备与近端2in1处于同一网络，且2in1可ping通远端设备IP。如不满足以上条件请勿使用该命令进行切换。
