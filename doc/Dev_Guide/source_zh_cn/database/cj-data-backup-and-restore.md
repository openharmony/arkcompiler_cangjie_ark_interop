...
键值型数据库，通过backup接口实现数据库备份，通过restore接口实现数据库恢复，通过deletebackup接口删除数据库备份。具体接口及功能，请参见[分布式键值数据库](../../../API_Reference/source_zh_cn/ArkData/cj-apis-distributed_kv_store.md)。
...
手动备份：通过调用[backup](../../../API_Reference/source_zh_cn/ArkData/cj-apis-relational_store.md#func-backupstring)接口实现数据库手动备份。示例如下：
