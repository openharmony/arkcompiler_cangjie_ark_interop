...
| --period | Set the event collection period (number of events per sample). Cannot be used with `-f`. |
...
| --exclude-tid | Thread IDs to exclude, separated by commas. Cannot be used with `-a`. |
| --exclude-thread | Thread names to exclude, separated by commas. Cannot be used with `-a`. |
| --offcpu | Track when threads are off CPU scheduling. |
| -j | Branch stack sampling. Filters include `any`, `any_call`, `any_ret`, `ind_call`, `ind_jmp`, `cond`, `call`. |
...
| --disable-callstack-expand | When `-s dwarf` is set, bypass the 64K stack limit. By default, call stacks are merged for more complete traces, though this may sometimes be inaccurate. |
...
| --symbol-dir | Path to the online symbol table files. |
...
| --restart | Collect performance metrics during application startup. If the process does not start within 30 seconds, recording exits. |
| --verbose | Output a more detailed report. |
| --control [command] | Collection command control parameters. Commands include `prepare`, `start`, `pause`, `resume`, `stop`. |
| --dedup_stack | Remove duplicate stacks from the recording. Cannot be used with `-a`. |
| --cmdline-size | Set the value of `/sys/kernel/tracing/saved_cmdlines_size` (range: 512-4096). |
| --report | Generate a call stack report after collection. Cannot be used with `-a`. |
...
Sample the process with PID 267 for 10 seconds using dwarf call stacks.
...
The following command monitors the performance counters of process 1273 on CPU 0 for 3 seconds.
...
This command primarily reads `perf.data` in raw format, allowing developers and testers to verify the correctness of the original sampled data.
...
| --syspath | Path to the symbol table files. |
| -i | Input file path. |
| -o | Output file path (outputs to screen if not set). |
| --elf | Output ELF file. |
| --proto | Output data in protobuf format. |
...
This command primarily displays sampled data (read from `perf.data`) and converts it into the required format (e.g., JSON or ProtoBuf).

**report Command Parameters:**

| Parameter | Description |
| -h/--help  | Help command. |
| --symbol-dir | Path to the symbol table file. |
| --limit-percent | Display only the top specified percentage of content. |
| -s | Enable stack trace mode. |
| --call-stack-limit-percent | Display only the top specified percentage of stack content. |
| -i | Path to the resource file, defaults to perf.data. |
| -o | Output file path. If not specified, outputs to the screen. |
| --proto | Output data in protobuf format. |
| --json | Output data in JSON format. |
| --diff | Display differences between two files specified with -i and --diff. |
...
| --sort [key1],[key2],[...] | Sort by specified keywords. |
| --hide_count | Hide numerical values in the report. |
| --dumpoptions | Dump command options. |
...
Example command to generate a standard report, limited to the top 1%:
...
General users can execute sampling operations via scripts to generate visual flame graphs. Tools can be obtained from the [developtools_hiperf repository](https://gitee.com/openharmony/developtools_hiperf/tree/master/script).

1. Sampling.

   Handled by command_script.py, which is a wrapper script for the report command.
   ...
   Sample a specified package, e.g., com.ohos.launch:
...
2. Collect symbol tables.
   ...
   Handled by recv_binary_cache.py, which primarily locates corresponding ELF files (used for stack traces or function name printing) based on related files and libraries recorded in perf.data and their buildids, within user-specified paths.

   ```shell
   usage: recv_binary_cache.py [-h] [-i PERF_DATA] [-l LOCAL_LIB_DIR [LOCAL_LIB_DIR ...]]
   ```

   Specify two symbol table path locations:
   ...
   Corresponding symbol table files will be copied to the binary_cache folder. The script prioritizes checking user-provided symbol table locations; if none are found, it copies files from the device.

3. Generate flame graphs.

   Handled by make_report.py, which exports sampled data into an HTML visualization page.
