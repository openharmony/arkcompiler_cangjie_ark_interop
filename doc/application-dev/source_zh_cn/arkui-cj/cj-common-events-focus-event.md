...
> **说明：**
>
> - [Popup](./cj-popup-and-menu-components-popup.md)组件在[focusable](../../../reference/source_zh_cn/arkui-cj/cj-universal-attribute-focus.md#focusablebool)属性（组件属性，非通用属性）为false的时候，不会有第2条特性。
> - NavBar、[NavDestination](../../../reference/source_zh_cn/arkui-cj/cj-navigation-switching-navdestination.md#navdestination)没有第3条特性，对于它们的走焦范围，是与它们的首个父层级页面相同的。

#### 根容器

根容器是[层级页面](#层级页面)内的概念，当某个[层级页面](#层级页面)首次创建并展示时，根据[层级页面](#层级页面)的特性，焦点会立即被该页面抢占。此时，该[层级页面](#层级页面)所在焦点链的末端节点将成为默认焦点，而这个默认焦点通常位于该[层级页面](#层级页面)的根容器上。

在缺省状态下，[层级页面](#层级页面)的默认焦点位于其根容器上，但开发者可以通过defaultFocus属性来自定义这一行为。
...
指开发者/用户主观行为导致的焦点移动，包括：使用外接键盘的按键走焦（TAB键/Shift+TAB键/方向键）、使用[requestFocus](../../../reference/source_zh_cn/arkui-cj/cj-universal-attribute-focus.md#static-func-requestfocusstring)申请焦点、[focusOnTouch](../../../reference/source_zh_cn/arkui-cj/cj-universal-attribute-focus.md#func-focusontouchbool)点击申请焦点等接口导致的焦点转移。

- 按键走焦

    - 前提：当前应用需处于焦点激活态。
    - 范围限制：按键走焦仅在当前获得焦点的[层级页面](#层级页面)内进行，详情请参见[层级页面](#层级页面)中的“走焦范围限制”部分。
    - 按键类型：
...
- requestFocus
    ...
    不可跨窗口，不可跨ArkUI实例申请焦点，可以跨[层级页面](#层级页面)申请焦点。
...
- [层级页面](#层级页面)切换：当发生[层级页面](#层级页面)切换时，如从一个页面跳转到另一个页面，当前页面的焦点将自动释放，新页面可能会根据预设逻辑自动获得焦点。
