# Menu Control

Bind a popup menu to a component. The popup menu displays menu items in a vertical list format, which can be triggered via long press, click, or right-click.

> **NOTE:**
> 
> - CustomBuilder does not support using bindMenu or bindContextMenu to pop up menus.
> - The text content of popup menus does not support long-press selection.
> - If the component is a draggable node and bindContextMenu is bound without specifying `preview`, the menu popup will display a floating drag preview, and the menu options will not avoid the preview. Developers can set `preview` or configure the target node as non-draggable based on usage scenarios.
> - The menu supports long-press for 500ms to pop up submenus and follows finger movement during press state.

## func bindContextMenu(() -> Unit, ResponseType)

```cangjie
public func bindContextMenu(builder!: () -> Unit, responseType!: ResponseType = ResponseType.LongPress): This
```

**Description:** Binds a menu to a component, triggered via long press or right-click. The popup menu items require customization.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 12

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| builder | () -> Unit | Yes | \- | **Named parameter.** Custom component. |
| responseType | [ResponseType](./cj-common-types.md#enum-responsetype) | No | ResponseType.LongPress | **Named parameter.** Menu trigger condition: long press or right-click. |

## func bindContextMenu(() -> Unit, ResponseType, ContextMenuOptions)

```cangjie
public func bindContextMenu(builder!: () -> Unit, responseType!: ResponseType, options!: ContextMenuOptions): This
```

**Description:** Binds a menu to a component, triggered via long press or right-click. The popup menu items require customization.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 12

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| builder | () -> Unit | Yes | \- | **Named parameter.** Custom component. |
| responseType | [ResponseType](./cj-common-types.md#enum-responsetype) | Yes | \- | **Named parameter.** Menu trigger condition: long press or right-click.<br> Initial value: ResponseType.LongPress. |
| options | [ContextMenuOptions](#class-contextmenuoptions) | Yes | \- | **Named parameter.** Configuration parameters for the popup menu. |

## func bindMenu(Array\<Action>)

```cangjie
public func bindMenu(menuList: Array<Action>): This
```

**Description:** Binds a menu to a component, which pops up upon click. The popup menu items support both icon + text arrangement and customization.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 12

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| menuList | Array\<[Action](#class-action)> | Yes | \- | Array configuring menu item icons and text. |

## func bindMenu(Array\<MenuElement>, ?MenuOptions)

```cangjie
public func bindMenu(content: Array<MenuElement>, options!: ?MenuOptions = None): This
```

**Description:** Binds a menu to a component, which pops up upon click. The popup menu items support both icon + text arrangement and customization.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 19

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| content | Array\<[MenuElement](#class-menuelement)> | Yes | \- | Array configuring menu item icons and text. |
| options | ?[MenuOptions](#class-menuoptions) | Yes | None | **Named parameter.** Configuration parameters for the popup menu. |

## func bindMenu(() -> Unit)

```cangjie
public func bindMenu(builder!: () -> Unit): This
```

**Description:** Binds a menu to a component, which pops up upon click. The popup menu items support both icon + text arrangement and customization.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 12

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| builder | () -> Unit | Yes | \- | **Named parameter.** Custom UI description. Use with @Builder. |

## class ContextMenuAnimationOptions

```cangjie
public open class ContextMenuAnimationOptions {
    public ContextMenuAnimationOptions(
        public var scale!: ?(Float64, Float64) = None,
        public var transition!: ?TransitionEffect = None,
        public var hoverScale!: ?(Float64, Float64) = None
    )
}
```

**Description:** Controls the start and end scaling ratios (relative to the original preview image) for long-press preview display animations.

**System Capability:** SystemCapability.ArkUI.ArkUI.Full

**Since:** 19

### var hoverScale

```cangjie
public var hoverScale: ?(Float64, Float64) = None
```

**Description:** Sets the start and end scaling ratios (relative to the original preview image) for the floating component screenshot animation in custom long-press scenarios, with a transition effect between the preview and the screenshot.

**Type:** ?(Float64, Float64)

**Access:** Read-write

**Since:** 19

### var scale

```cangjie
public var scale: ?(Float64, Float64) = None
```

**Description:** Start and end scaling ratios relative to the original preview image.

**Type:** ?(Float64, Float64)

**Access:** Read-write

**Since:** 19

### var transition

```cangjie
public var transition: ?TransitionEffect = None
```

**Description:** Sets the transition effect for menu display and exit.

**Type:** TransitionEffect

**Access:** Read-write

**Since:** 19

### ContextMenuAnimationOptions(?(Float64,Float64), ?TransitionEffect, ?(Float64,Float64))

```cangjie
public ContextMenuAnimationOptions(
    public var scale!: ?(Float64, Float64) = None,
    public var transition!: ?TransitionEffect = None,
    public var hoverScale!: ?(Float64, Float64) = None
)
```

**Description:** Creates a ContextMenuAnimationOptions object.

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| scale | ?(Float64, Float64) | No | None | **Named parameter.** Start and end scaling ratios relative to the original preview image.<br> **Note:** Scaling ratios should be set based on actual development scenarios. Recommended values are less than the preview image width or layout constraints. |
| transition | ?TransitionEffect | No | None | **Named parameter.** Transition effect for menu display and exit.<br> **Note:** During menu exit animation, switching between portrait and landscape modes will cause the menu to avoid. Secondary menus do not inherit custom effects. Clicking secondary menus is allowed during popup but not during exit animation. See TransitionEffect for details. |
| hoverScale | ?(Float64, Float64) | No | None | **Named parameter.** Start and end scaling ratios (relative to the original preview image) for the floating component screenshot animation in custom long-press scenarios, with a transition effect.<br> **Note:** Invalid if scaling ratio ≤ 0.<br> Does not take effect if the `transition` interface is used.<br> If used with the `scale` interface, the `scale` start value is ignored.<br> For optimal experience, the final preview size should not be smaller than the original component screenshot. Current preview animation dimensions are affected by component screenshot and custom preview size. Ensure display effects based on actual usage. |

## class ContextMenuOptions

```cangjie
public open class ContextMenuOptions {
    public ContextMenuOptions(
        public var offset!: Position = Position(0.0, 0.0),
        public var placement!: Option<Placement> = Option.None,
        public var enableArrow!: Bool = false,
        public var arrowOffset!: Length = 0.vp,
        public var preview!: Option<() -> Unit> = Option.None,
        public var previewAnimationOptions!: ?ContextMenuAnimationOptions = None,
        public var onAppear!: ?() -> Unit = None,
        public var onDisappear!: ?() -> Unit = None,
        public var aboutToAppear!: ?() -> Unit = None,
        public var aboutToDisappear!: ?() -> Unit = None,
        public var backgroundColor!: ResourceColor = Color.TRANSPARENT,
        public var backgroundBlurStyle!: BlurStyle = BlurStyle.COMPONENT_ULTRA_THICK,
        public var transition!: ?TransitionEffect = None
    )
}
```

**Description:** Configuration parameters for popup menus.

**Since:** 19

### var aboutToAppear

```cangjie
public var aboutToAppear:?() -> Unit = None
```

**Description:** Callback event before menu display animation.

**Type:** ?()->Unit

**Access:** Read-write

**Since:** 19

### var aboutToDisappear

```cangjie
public var aboutToDisappear:?() -> Unit = None
```

**Description:** Callback event before menu exit animation.

**Type:** ?()->Unit

**Access:** Read-write

**Since:** 19

### var arrowOffset

```cangjie
public var arrowOffset: Length = 0.vp
```

**Description:** Arrow offset on the menu. The offset must be valid and greater than 0 when converted to a specific value. Additionally, this value will not cause the arrow to exceed the safe distance around the menu when applied.

**Type:** [Length](./cj-common-types.md#interface-length)

**Access:** Read-write

**Since:** 19

### var backgroundBlurStyle

```cangjie
public var backgroundBlurStyle: BlurStyle = BlurStyle.COMPONENT_ULTRA_THICK
```

**Description:** Blur material for the popup backdrop.

**Type:** [BlurStyle](./cj-universal-attribute-background.md#enum-blurstyle)

**Access:** Read-write

**Since:** 19

### var backgroundColor

```cangjie
public var backgroundColor: ResourceColor = Color.TRANSPARENT,
```

**Description:** Color of the popup backdrop.

**Type:** [ResourceColor](./cj-common-types.md#interface-resourcecolor)

**Access:** Read-write

**Since:** 19

### var enableArrow

```cangjie
public var enableArrow:Bool = false
```

**Description:** Whether to display an arrow. The arrow will not be shown if the menu size or position cannot accommodate it.

**Type:** Bool

**Access:** Read-write

**Since:** 19

### var offset

```cangjie
public var offset: Position = Position(0.0, 0.0)
```
**Function:** The offset of the menu popup position, ensuring the menu does not display outside the screen boundaries.

**Type:** [Position](./cj-common-types.md#class-position)

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var onAppear

```cangjie
public var onAppear:?() -> Unit = None
```

**Function:** Event callback when the menu pops up.

**Type:** ?()->Unit

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var onDisappear

```cangjie
public var onDisappear:?() -> Unit = None
```

**Function:** Event callback when the menu disappears.

**Type:** ?()->Unit

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var placement

```cangjie
public var placement: Option<Placement> = Option.None
```

**Function:** The preferred display position of the menu component. If the current position cannot accommodate it, the position will be automatically adjusted.

**Type:** Option\<[Placement](./cj-common-types.md#enum-placement)>

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var preview

```cangjie
public var preview: Option<() -> Unit> = Option.None
```

**Function:** The preview content style for long-press floating menus or menus displayed using `bindContextMenu`, customizable by the user.

**Type:** Option\<() -> Unit>

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var previewAnimationOptions

```cangjie
public var previewAnimationOptions: ?ContextMenuAnimationOptions = None
```

**Function:** Controls the start and end magnification of the long-press preview animation (relative to the original preview image scale).

**Type:** ?[ContextMenuAnimationOptions](#class-contextmenuanimationoptions)

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var transition

```cangjie
public var transition: ?TransitionEffect = None
```

**Function:** Sets the transition effect for menu display and exit.

**Type:** ?TransitionEffect

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### ContextMenuOptions(Position, Option\<Placement>, Bool, Length, Option\<() -> Unit>, ?ContextMenuAnimationOptions, ?() -> Unit, ?() -> Unit, ?() -> Unit, ?() -> Unit, ResourceColor, BlurStyle, ?TransitionEffect)

```cangjie
public ContextMenuOptions(
    public var offset!: Position = Position(0.0, 0.0),
    public var placement!: Option<Placement> = Option.None,
    public var enableArrow!: Bool = false,
    public var arrowOffset!: Length = 0.vp,
    public var preview!: Option<() -> Unit> = Option.None,
    public var previewAnimationOptions!: ?ContextMenuAnimationOptions = None,
    public var onAppear!: ?() -> Unit = None,
    public var onDisappear!: ?() -> Unit = None,
    public var aboutToAppear!: ?() -> Unit = None,
    public var aboutToDisappear!: ?() -> Unit = None,
    public var backgroundColor!: ResourceColor = Color.TRANSPARENT,
    public var backgroundBlurStyle!: BlurStyle = BlurStyle.COMPONENT_ULTRA_THICK,
    public var transition!: ?TransitionEffect = None
)
```

**Function:** Creates a `ContextMenuOptions` object.

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| offset | [Position](./cj-common-types.md#class-position) | No | Position(0.0, 0.0) | **Named parameter.** The offset of the menu popup position, ensuring the menu does not display outside the screen boundaries.<br> **Note:**<br> When the menu type is a popup relative to the parent component area, the width or height of the area is automatically included in the offset based on the `placement` attribute.<br> When the menu appears above the parent component (`placement` set to `Placement.TopLeft`, `Placement.Top`, or `Placement.TopRight`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu upward.<br> When the menu appears below the parent component (`placement` set to `Placement.BottomLeft`, `Placement.Bottom`, or `Placement.BottomRight`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu downward.<br> When the menu appears to the left of the parent component (`placement` set to `Placement.LeftTop`, `Placement.Left`, or `Placement.LeftBottom`), a positive `x` value offsets the menu to the left, and a positive `y` value offsets the menu downward.<br> When the menu appears to the right of the parent component (`placement` set to `Placement.RightTop`, `Placement.Right`, or `Placement.RightBottom`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu downward.<br> If the menu adjusts its display position (inconsistent with the initial `placement` direction), the `offset` value becomes invalid. |
| placement | Option\<[Placement](./cj-common-types.md#enum-placement)> | No | Option.None | **Named parameter.** The preferred display position of the menu component. If the current position cannot accommodate it, the position will be automatically adjusted.<br> **Note:**<br> If `placement` is set to `undefined`, `null`, or not set, it is treated as unset. When using `bindMenu`, the default is `Placement.BottomLeft`. |
| enableArrow | Bool | No | false | **Named parameter.** Whether to display an arrow. If the menu size or position cannot accommodate the arrow, it will not be displayed.<br> **Note:** <br> When `enableArrow` is `true`, if `placement` is unset or invalid, the menu defaults to displaying above the target. Otherwise, it follows the `placement` priority. If the current position cannot accommodate it, the position is automatically adjusted. If `enableArrow` is `undefined`, no arrow is displayed. |
| arrowOffset | [Length](./cj-common-types.md#interface-length) | No | 0.vp | **Named parameter.** The offset of the arrow at the menu. The offset must be valid and greater than 0 when converted to a specific value. Additionally, this value will not cause the arrow to exceed the safe distance around the menu.<br> Unit: vp<br> **Note:**<br> The safe distance around the menu is the sum of the menu's corner radius and half the arrow width.<br> The offset is calculated based on the configured `placement` (horizontal or vertical).<br> When the arrow is on the horizontal side of the menu, the offset is the distance from the arrow to the leftmost safe distance. When the arrow is on the vertical side, the offset is the distance from the arrow to the topmost safe distance.<br> Default arrow positions vary by `placement`:<br> - For `Placement.Top` or `Placement.Bottom`, the arrow is centered horizontally.<br> - For `Placement.Left` or `Placement.Right`, the arrow is centered vertically.<br> - For `Placement.TopLeft` or `Placement.BottomLeft`, the arrow is on the horizontal side, offset from the left edge by the safe distance.<br> - For `Placement.TopRight` or `Placement.BottomRight`, the arrow is on the horizontal side, offset from the right edge by the safe distance.<br> - For `Placement.LeftTop` or `Placement.RightTop`, the arrow is on the vertical side, offset from the top edge by the safe distance.<br> - For `Placement.LeftBottom` or `Placement.RightBottom`, the arrow is on the vertical side, offset from the bottom edge by the safe distance. |
| preview | Option\<() -> Unit> | No | Option.None | **Named parameter.** The preview content style for long-press floating menus or menus displayed using `bindContextMenu`, customizable by the user.<br> **Note:** <br> - Not supported when `responseType` is `ResponseType.RightClick`; no preview content will be displayed.<br> - When `preview` is unset, the `enableArrow` parameter takes effect.<br> - When `preview` is set to `CustomBuilder`, no arrow is displayed even if `enableArrow` is `true`. |
| previewAnimationOptions | ?[ContextMenuAnimationOptions](#class-contextmenuanimationoptions) | No | None | **Named parameter.** Controls the start and end magnification of the long-press preview animation (relative to the original preview image scale).<br> **Note:**<br> Magnification settings ≤ 0 are ignored. |
| onAppear | ?()->Unit | No | None | **Named parameter.** Event callback when the menu pops up. |
| onDisappear | ?()->Unit | No | None | **Named parameter.** Event callback when the menu disappears. |
| aboutToAppear | ?()->Unit | No | None | **Named parameter.** Event callback before the menu display animation. |
| aboutToDisappear | ?()->Unit | No | None | **Named parameter.** Event callback before the menu exit animation. |
| backgroundColor | [ResourceColor](./cj-common-types.md#interface-resourcecolor) | No | Color.TRANSPARENT | **Named parameter.** The background color of the popup. |
| backgroundBlurStyle | [BlurStyle](./cj-universal-attribute-background.md#enum-blurstyle) | No | BlurStyle.COMPONENT_ULTRA_THICK | **Named parameter.** The blur material of the popup background. |
| transition | ?TransitionEffect | No | None | **Named parameter.** The transition effect for menu display and exit.<br> **Note:**<br> During the exit animation, the menu will avoid screen rotation. Secondary menus do not inherit custom animations. Secondary menus can be clicked during the popup animation but not during the exit animation.<br> For details, refer to the `TransitionEffect` object description. |

## class MenuElement

```cangjie
public class MenuElement {
    public init(
        value: String,
        action: () -> Unit,
        icon!: String = "",
        enabled!: Bool = false
    )
}
```

**Function:** Configures menu item icons and text.

**Initial Version:** 19

### init(String, () -> Unit, String, Bool)

```cangjie
public init(
    value: String,
    action: () -> Unit,
    icon!: String = "",
    enabled!: Bool = false
)
```

**Function:** Creates a `MenuElement` object.

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| value | String | Yes | \- | The text of the menu item. |
| action | ()->Unit | Yes | \- | Event callback when the menu item is clicked. |
| icon | String | No | "" | **Named parameter.** The icon of the menu item. |
| enabled | Bool | No | true | **Named parameter.** Whether the menu item is interactive. |

## class MenuOptions

```cangjie
public class MenuOptions <: ContextMenuOptions {
    public var title: ?String
    public var showInSubWindow: ?Bool
    public init(
        offset!: Position = Position(0.0, 0.0),
        placement!: Option<Placement> = Option.None,
        enableArrow!: Bool = false,
        arrowOffset!: Length = 0.vp,
        preview!: Option<() -> Unit> = Option.None,
        previewAnimationOptions!: ?ContextMenuAnimationOptions = None,
        onAppear!: ?() -> Unit = None,
        onDisappear!: ?() -> Unit = None,
        aboutToAppear!: ?() -> Unit = None,
        aboutToDisappear!: ?() -> Unit = None,
        backgroundColor!: ResourceColor = Color.TRANSPARENT,
        backgroundBlurStyle!: BlurStyle = BlurStyle.COMPONENT_ULTRA_THICK,
        transition!: ?TransitionEffect = None,
        title!: ?String = None,
        showInSubWindow!: ?Bool = None
    )
}
```

**Function:** Configures parameters for popup menus.

**Parent Type:**

- [ContextMenuOptions](#class-contextmenuoptions)

### var showInSubWindow

```cangjie
public var showInSubWindow:?Bool = None
```

**Function:** The title of the menu.

**Type:** ?Bool

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### var title

```cangjie
public var title:?String = None
```

**Function:** Whether to display the menu in a sub-window.

**Type:** ?String

**Read-Write Capability:** Read-Write

**Initial Version:** 19

### init(Position, Option\<Placement>, Bool, Length, Option\<() -> Unit>, ?ContextMenuAnimationOptions, ?() -> Unit, ?() -> Unit, ?() -> Unit, ?() -> Unit, ResourceColor, BlurStyle, ?TransitionEffect, ?String, ?Bool)

```cangjie
public init(
    offset!: Position = Position(0.0, 0.0),
    placement!: Option<Placement> = Option.None,
    enableArrow!: Bool = false,
    arrowOffset!: Length = 0.vp,
    preview!: Option<() -> Unit> = Option.None,
    previewAnimationOptions!: ?ContextMenuAnimationOptions = None,
    onAppear!: ?() -> Unit = None,
    onDisappear!: ?() -> Unit = None,
    aboutToAppear!: ?() -> Unit = None,
    aboutToDisappear!: ?() -> Unit = None,
    backgroundColor!: ResourceColor = Color.TRANSPARENT,
    backgroundBlurStyle!: BlurStyle = BlurStyle.COMPONENT_ULTRA_THICK,
    transition!: ?TransitionEffect = None,
    title!: ?String = None,
    showInSubWindow!: ?Bool = None
)
```

**Function:** Creates a `MenuOptions` object.

**Parameters:**

| Parameter Name | Type | Required | Default Value | Description |
|:---|:---|:---|:---|:---|
| offset | [Position](./cj-common-types.md#class-position) | No | Position(0.0, 0.0) | **Named parameter.** The offset of the menu popup position, ensuring the menu does not display outside the screen boundaries.<br> **Note:**<br> When the menu type is a popup relative to the parent component area, the width or height of the area is automatically included in the offset based on the `placement` attribute.<br> When the menu appears above the parent component (`placement` set to `Placement.TopLeft`, `Placement.Top`, or `Placement.TopRight`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu upward.<br> When the menu appears below the parent component (`placement` set to `Placement.BottomLeft`, `Placement.Bottom`, or `Placement.BottomRight`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu downward.<br> When the menu appears to the left of the parent component (`placement` set to `Placement.LeftTop`, `Placement.Left`, or `Placement.LeftBottom`), a positive `x` value offsets the menu to the left, and a positive `y` value offsets the menu downward.<br> When the menu appears to the right of the parent component (`placement` set to `Placement.RightTop`, `Placement.Right`, or `Placement.RightBottom`), a positive `x` value offsets the menu to the right, and a positive `y` value offsets the menu downward.<br> If the menu adjusts its display position (inconsistent with the initial `placement` direction), the `offset` value becomes invalid. |
| placement | Option\<[Placement](./cj-common-types.md#enum-placement)> | No | Option.None | **Named parameter.** The preferred display position of the menu component. If the current position cannot accommodate it, the position will be automatically adjusted.<br> **Note:**<br> If `placement` is set to `undefined`, `null`, or not set, it is treated as unset. When using `bindMenu`, the default is `Placement.BottomLeft`. |
| enableArrow | Bool | No | false | **Named parameter.** Whether to display an arrow. If the menu size or position cannot accommodate the arrow, it will not be displayed.<br> **Note:** <br> When `enableArrow` is `true`, if `placement` is unset or invalid, the menu defaults to displaying above the target. Otherwise, it follows the `placement` priority. If the current position cannot accommodate it, the position is automatically adjusted. If `enableArrow` is `undefined`, no arrow is displayed. |
| arrowOffset | [Length](./cj-common-types.md#interface-length) | No | 0.vp | **Named parameter.** The offset of the arrow at the menu. The offset must be valid and greater than 0 when converted to a specific value. Additionally, this value will not cause the arrow to exceed the safe distance around the menu.<br> Unit: vp<br> **Note:**<br> The safe distance around the menu is the sum of the menu's corner radius and half the arrow width.<br> The offset is calculated based on the configured `placement` (horizontal or vertical).<br> When the arrow is on the horizontal side of the menu, the offset is the distance from the arrow to the leftmost safe distance. When the arrow is on the vertical side, the offset is the distance from the arrow to the topmost safe distance.<br> Default arrow positions vary by `placement`:<br> - For `Placement.Top` or `Placement.Bottom`, the arrow is centered horizontally.<br> - For `Placement.Left` or `Placement.Right`, the arrow is centered vertically.<br> - For `Placement.TopLeft` or `Placement.BottomLeft`, the arrow is on the horizontal side, offset from the left edge by the safe distance.<br> - For `Placement.TopRight` or `Placement.BottomRight`, the arrow is on the horizontal side, offset**Read-Write Capability:** Read-only  

**Initial Version:** 12  

### var value  

```cangjie  
let value: String  
```  

**Function:** Menu item text.  

**Type:** String  

**Read-Write Capability:** Read-only  

**Initial Version:** 12  

### Action(String, () -> Unit)  

```cangjie  
public Action(  
    let value!: String,  
    let action!: () -> Unit  
)  
```  

**Function:** Creates an Action object.  

**Initial Version:** 12  

**Parameters:**  

| Parameter Name | Type | Required | Default Value | Description |  
|:--------------|:-----|:---------|:-------------|:------------|  
| value | String | Yes | \- | **Named parameter.** Menu item text. |  
| action | () -> Unit | Yes | \- | **Named parameter.** Callback for menu item click event. |  

## Example Code  

### Example 1 (Display Custom Menu)  

This example demonstrates how to configure `CustomBuilder` via `bindMenu` to display a custom menu.  

<!-- run -->  

```cangjie  

package ohos_app_cangjie_entry  
import kit.UIKit.*  
import ohos.state_macro_manage.*  

@Entry  
@Component  
class EntryView {  
    @Builder  
    func builder() {  
        Column {  
            Button("Builder Content")  
                .width(300.px)  
                .onClick({  
                    evt => nativeLog("Button in Builder clicked")  
                })  
        }  
        .width(300.px)  
    }  

    func build() {  
        Column(20) {  
            Button("BindMenu").bindMenu(  
                [  
                    Action(  
                        value: "Menu 1",  
                        action: {  
                            => nativeLog("Menu 1 clicked")  
                        }  
                    ),  
                    Action(  
                        value: "Menu 2",  
                        action: {  
                            => nativeLog("Menu 2 clicked")  
                        }  
                    )  
                ]  
            )  

            Button("BindMenu-Custom")  
                .bindMenu(builder: builder)  
            Button("BindContextMenu-Long Press")  
                .bindContextMenu(builder: builder)  
        }  
    }  
}  
```  

![uni_bind_menu](figures/uni_bind_menu.png)  

### Example 2 (Display Standard Menu)  

This example demonstrates how to configure `MenuElement` via `bindMenu` to display a standard menu.  

<!-- run -->  

```cangjie  

package ohos_app_cangjie_entry  

import kit.UIKit.*  
import ohos.state_macro_manage.*  

@Entry  
@Component  
class EntryView {  
    func build() {  
        Scroll() {  
            Column(10) {  
                Button("BindMenu").bindMenu(  
                    [  
                        MenuElement(  
                            "Menu 1",  
                            {  
                                => AppLog.error("MenuElement test: Menu 1 clicked")  
                            }  
                        ),  
                        MenuElement(  
                            "Menu 2",  
                            {  
                                => AppLog.error("MenuElement test: Menu 2 clicked")  
                            },  
                            icon: "data:image/png;base64,",
                            enabled: true
                        )
                    ]
                )

                Button("BindMenu").bindMenu(
                    [
                        MenuElement(
                            "Menu 1",
                            {
                                => AppLog.error("MenuElement test: Menu 1 clicked")
                            }
                        ),
                        MenuElement(
                            "Menu 2",
                            {
                                => AppLog.error("MenuElement test: Menu 2 clicked")
                            },
                            icon: "data:image/png;base64,",
                            enabled: true
                        )
                    ],
                    options: MenuOptions(
                        enableArrow: true,
                        onAppear: {=> AppLog.error("MenuOptions test: onAppear")},
                        onDisappear: {=> AppLog.error("MenuOptions test: onDisappear")},
                        aboutToAppear: {=> AppLog.error("MenuOptions test: aboutToAppear")},
                        aboutToDisappear: {=> AppLog.error("MenuOptions test: aboutToDisappear")},
                        title: "MenuOptions",
                        showInSubWindow: true
                    )
                )
            }
        }
    }
}
```

![uni_bind_menu](figures/uni_bind_menu.gif)